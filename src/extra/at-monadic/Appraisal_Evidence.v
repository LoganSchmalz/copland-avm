...
Proof.
  ...
  Axiom cvm_evidence_core_at : forall u t p tok et bits bits' s j, 
  doRemote_uuid_payload u (jsonToStr (requestToJson (REQ t tok bits))) = resultC s ->
  strToJson s = resultC j ->
  jsonToResponse j = resultC (RES bits') ->
  cvm_evidence_core (copland_compile t) p (evc bits et) = evc bits' (eval t p et).
