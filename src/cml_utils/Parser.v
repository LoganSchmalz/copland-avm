Require Import String Ascii List Nat Bool Program.
...
(* 
  ...
  Proof.
    ...
    pose proof (consuming_parser_skipMany_rec space (consuming_parser_label "Expected an ascii whitespace character." (satisfy isSpace) (consuming_parser_satisfy isSpace))).
    inv H1. eauto.
    all: fail.
...
(* TODO: This is not a consuming_parser BY SPEC!
  ...
  (* sepBy: ('a, 'b) parser -> ('c, 'b) parser -> ('a list, 'b) parser
    ...
    * failure.
    *)
  Program Fixpoint sepBy_rec {A B : Type} `{HA : EqClass A} `{HB : EqClass B}
      (p : parser A B) (sepp : parser A B) (CP : consuming_parser p) (CSP : consuming_parser sepp)
  ...
  (* manyTill: ('a, 'b) parser -> ('c, 'b) parser -> ('a list, 'b) parser
    ...
    * and then applies `p`.
    *)
  Program Fixpoint manyTill_rec {A B : Type} `{HA : EqClass A} `{HB : EqClass B}
      (p : parser A B) (endP : parser A B) (CP : consuming_parser p) (CEP : consuming_parser endP)
      (cs : list B) (line col : nat) {measure (length cs)} : (res (list A) string * list B * nat * nat) := 
    match cs with
    | nil => (Err "Cannot do manyTill on empty input", cs, line, col)
    | c::cs' =>
      match (endP (cs, line, col)) with
      | (Ok _, cs', line', col') => (Ok nil, cs', line', col')
      | (Err _, _, _, _) =>
          match (p (cs, line, col)) with
          | (Err err, cs', line', col') => (Err err, cs', line', col')
          | (Ok x, cs', line', col') =>
              match (manyTill_rec p endP CP CEP cs' line' col') with
              | (Err m, cs'', line'', col'') => (Ok (x :: nil), cs', line', col')
              | (Ok rec_rs, cs'', line'', col'') =>
                  (Ok (x :: rec_rs), cs'', line'', col'')
              end
          end
  ...
  Next Obligation.
    intros; subst.
    destruct CP, CEP.
    assert (p (c :: cs'0, line, col) = (Ok x, cs', line', col')). eauto.
    assert (endP (c :: cs'0, line, col) = (Err wildcard', wildcard'0, wildcard'1, wildcard'2)). eauto.
    pose proof (consuming_always0 (c :: cs'0) line col cs' line' col' x H).
    destruct H1; eauto; subst; cbv; lia.
  Qed.
  Next Obligation.
    auto with *.
  Qed.
  ...
  Definition followedBy {A B C : Type} `{EqClass A, EqClass B, EqClass C} (p : parser A C) 
      (q : parser B C) (CP : consuming_parser p) (CQ : consuming_parser q) : (parser A C) :=
    bind 
      p 
      (fun a => 
        (parser_return a q CQ
        )
      )
      CP
      (fun a => (consuming_parser_parser_return a q CQ)).

  Lemma consuming_parser_followedBy : forall {A B C : Type} `{EqClass A, EqClass B, EqClass C} (p : parser A C) 
      (q : parser B C) (CP : consuming_parser p) (CQ : consuming_parser q),
    consuming_parser (followedBy p q CP CQ).
  Proof.
    intros. eapply consuming_parser_bind.
  Qed.

  (* ('a, char) parser -> ('a, char) parser *)
  (* Token-based parsing consumes all whitespace after parsing *)
  Definition token {A : Type} `{EqClass A, EqClass cml_unit} (p : parser A ascii) 
      (CP : consuming_parser p) : (parser A ascii) := 
    followedBy p spaces CP consuming_parser_spaces.

  Lemma consuming_parser_token : forall {A : Type} `{EqClass A, EqClass cml_unit} 
      (p : parser A ascii) (CP : consuming_parser p),
    consuming_parser (token p CP).
  Proof.
    intros. eapply consuming_parser_followedBy.
  Qed.

  (* (string, char) parser *)
  Definition symbol `{EqClass cml_unit} (s : string) (HS : s <> "") : (parser string ascii) := 
    token 
      (parser_string s HS)
      (consuming_parser_parser_string s HS).

  Lemma consuming_parser_symbol : forall `{EqClass cml_unit} (s : string) (HS : s <> ""),
    consuming_parser (symbol s HS).
  Proof.
